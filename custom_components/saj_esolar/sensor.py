...
async def async_setup_platform(
    ...
    session = async_create_clientsession(
        hass, verify_ssl=config.get("provider_ssl")
    ...
    for description in SENSOR_TYPES:
        if description.key in config[CONF_RESOURCES]:
            sensor = SAJeSolarMeterSensor(
                description, data, config.get(CONF_SENSORS), config.get(CONF_PLANT_ID)
            )
...
class EsolarProvider:
    ...
    def __init__(self, host, path, protocol) -> None:
...
class SAJeSolarMeterData:
    ...
    async def async_update(self):
        ...
        _LOGGER.debug(
            self._session.cookie_jar.filter_cookies(self._provider.getBaseDomain())
        )
        ...
        if response_logout.status != 200:
            _LOGGER.error("%s returned %s", response_logout.url, response_logout.status)
            return
...
class SAJeSolarMeterSensor(SensorEntity):
    ...
    def __init__(
        self, description: SensorEntityDescription, data, sensors, plant_id
    ) -> None:
